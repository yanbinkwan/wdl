  ...
  var TaskItem = (function () {
    ...
    var ins = function ins(selection) {
      fetch("http://localhost:8000/task/list", {
        ...
        selection.append("ul").attr("class", "list-group").selectAll("li").data(tasks).join("li").attr("class", "list-group-item").text(function (d) {
          return d.type === "task" ? d.call_function : d.label;
  ...
  function dialog() {
    ...
    ins.showup = function () {
      ...
      data.outputParams && data.outputParams.forEach(function (p) {
        ...
        select("#output-label-".concat(data.id)).on("input", function (el) {
          var val = el.target.value;
          p.label = val;
        });
  ...
  function TaskNode () {
    ...
    var ins = function ins(selection) {
      ...
      group.selectAll("text").data(function (d) {
        ...
        return d;
      }).attr("y", 26).attr("dx", 10).attr("text-anchor", "middle").attr("font-size", "5").on("contextmenu", function (event) {
        event.preventDefault();
      });
      group.selectAll("rect.svg-container").data([null]).join("rect").attr("class", "svg-container").attr("height", 20).attr("width", 20).attr("rx", 2).attr("ry", 2).attr("fill", "#272643");
      group.selectAll("image").data(function (d) {
        return [d];
      }).join("image").attr("xlink:href", img).attr("width", 15).attr("height", 15).attr("x", 2.5).attr("y", 2.5).on("click", function (d, d1) {
        dg.data(d1).showup();
      ...
      group.selectAll("circle.inputs").data(function (d) {
        return d.inputParams || [];
      }).join("circle").attr("class", "inputs").attr("cx", function (d) {
        d.x = 0.1;
        return d.x;
      }).attr("cy", function (d, i, x) {
        var y = 20 / (x.length + 1);
        d.y = (i + 1) * y - 1.4 / 4;
        return d.y;
      }).attr("r", function (d) {
        ...
        return d.linked ? "#272643" : null;
      }).attr("stroke-width", function (d) {
        return d.linked ? "0.7" : null;
      });
      group.selectAll("circle.outputs").data(function (d) {
        return d.outputParams || [];
      }).join("circle").attr("class", "outputs").attr("cx", function (d) {
        d.x = 20;
        return d.x;
      }).attr("cy", function (d, i, x) {
        var y = 20 / (x.length + 1);
    ...
    var moveNode = function moveNode() {
      function dragstarted() {
        select(this).attr("class", "node-g grabing");
      }
    ...
    var linkNode = function linkNode(selection) {
      ...
      function dragstarted(event, d) {
        ...
        linkInfo.source = [x, y];
        selection.append("path").data([linkInfo]).attr("stroke", "#85c9d1").attr("fill", "none").attr("class", linkInfo.id);
      }
      function dragged(event, d) {
        var parentData = getParentData(d.pid);
        var x = event.x + parentData.x;
      ...
      function dragended(event, d) {
        ...
        var x = event.x + parentData.x;
        var y = event.y + parentData.y;
        var hasLinked = selectAll(".node-g .inputs").filter(function (inputD) {
          var parentD = getParentData(inputD.pid);
          var xp = parentD.x + inputD.x + inputD.r - x;
          var yp = parentD.y + inputD.y + inputD.r - y;
          var round = inputD.r * 2;
          return xp >= 0 && xp <= round && yp >= 0 && yp <= round && !inputD.linked;
        });
        if (hasLinked.size() > 0) {
          d.links = d.links || [];
    ...
    _createClass(Generator, [{
      ...
      value: function generate() {
        ...
        this.root.children.forEach(function (child) {
          ...
          switch (type) {
            case "call":
              s += " \tcall ".concat(child.task.label, ".").concat(child.task.call_function, " { input: ").concat(child.input.map(function (i, index) {
                return "".concat(i.label, "=").concat(child.output[index]);
              }).join(","), " } \n");
              break;
            case "output":
              s += " \toutput {\n            ".concat(child.input.type, " ").concat(child.input.label, "=").concat(child.output, " \n        }\n");
              break;
            case "input":
        ...
        str = "\n".concat(imports, "\nworkflow ").concat(this.root.label, " {\n").concat(s, "\n}");
        console.log(str);
      }
    }], [{
      key: "create",
  ...
  function TaskList (selection) {
    ...
    var handleLinkEvent = function handleLinkEvent(params) {
      ...
      var hasCalled = generator.root.children.find(function (child) {
        return child.type === "call" && child.task.id === target.id;
      });
      if (!hasCalled) {
    ...
    var TaskIns = TaskNode().on("link", handleLinkEvent).on("delete", handleTaskDeleteEvent);
