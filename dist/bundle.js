(function () {
  ...
  function WdlCanvas () {
    ...
    var view = svg.append("g").attr("class", "view").attr("x", 0.5).attr("y", 0.5).attr("width", width - 1).attr("height", height - 1);
    var z = zoom().filter(function (event) {
      event.preventDefault();
      return (!event.ctrlKey || event.type === "wheel") && !event.button;
    ...
    svg.call(z);
    view.append("g").attr("class", "g-dots").selectAll("circle.dot").data(square_grid()).join("circle").attr("class", "dot").attr("cx", function (d) {
      return d.x;
    }).attr("cy", function (d) {
      return d.y;
    }).attr("r", function (d) {
  ...
  function square_grid() {
    var s = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1900;
    var n = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 180;
  ...
  var TaskItem = (function () {
    ...
    var ins = function ins(selection) {
      selection.append("ul").attr("class", "list-group").selectAll("li").data(data).join("li").attr("class", "list-group-item").text(function (d) {
        return d.name;
  ...
  function dialog() {
    ...
    ins.showup = function (callback) {
      ...
      select(".add-button").on("click", function () {
        data.outputParams.push(_objectSpread2(_objectSpread2({}, data.outputParams[0]), {}, {
  ...
  var ContextMenu = function ContextMenu() {
    ...
    var ins = function ins(selection) {
      var options = ["删除", "修改"];
      var container = selection.selectAll(".context_menu").data([null]).join("div").classed("context_menu", true).style("left", x + "px").style("top", y + "px");
  ...
  function TaskNode () {
    ...
    var ins = function ins(selection) {
      var container = selection.select(".view").selectAll("g.container").data(data).join("g").classed("container", true).attr("transform", function (d) {
        return "translate(".concat(d.x, ", ").concat(d.y, ")");
      }).call(moveNode());
      container.selectAll("rect").data(function (d) {
        return [d];
      }).join("rect").classed("panel", true).attr("height", panelHeight).attr("width", panelWidth).attr("rx", 2.5).attr("ry", 2.5).attr("stroke", "#676785").attr("stroke-width", 0.5).attr("fill", "#313147").on("click", function (d, d1) {
        dg.data(d1).showup(function () {
          dispatch$1.call("update");
        });
      }).on("contextmenu", function (event, d) {
        event.preventDefault();
        select("#app").call(ContextMenu().x(event.pageX).y(event.pageY).on("delete", function (_ref) {
          var ins = _ref.ins;
      ...
      }).attr("text-anchor", "end").attr("dx", panelWidth - 5).attr("y", 8).attr("font-size", "4").attr("fill", "#e9ecff");
      container.append("text").text(function (d) {
        return d.name;
      }).attr("text-anchor", "start").attr("y", 8).attr("dx", 5).attr("font-size", "4").attr("fill", "#e9ecff").style("font-weight", "bold");
      container.append("text").text(function (d) {
        return d.callFunction;
      }).attr("y", 16).attr("dx", 5).attr("font-size", "4").attr("fill", "#e9ecff");
      container.selectAll("circle.inputs").data(function (d) {
        return d.inputParams || [];
      }).join("rect").classed("inputs", true).attr("width", streamSize).attr("height", streamSize).attr("rx", 1).attr("ry", 1).attr("x", function (d) {
        d.x = -2.5;
        return d.x;
        ...
        return d.linked ? "#85c9d1" : "#272643";
      }).attr("stroke", function (d) {
        return d.linked ? "#656593" : "#656593";
      }).attr("stroke-width", 0.4);
      container.selectAll("circle.outputs").data(function (d) {
        return d.outputParams || [];
      }).join("rect").attr("class", "outputs").attr("width", streamSize).attr("height", streamSize).attr("rx", 1).attr("ry", 1).attr("x", function (d) {
        d.x = panelWidth - 1.5;
        return d.x;
      }).attr("y", function (d, i, x) {
        var y = 38 / (x.length + 1);
        ...
        return d.linked ? "#656593" : "#656593";
      }).attr("stroke-width", 0.4).call(linkNode(selection));
    ...
    var linkNode = function linkNode(selection) {
      var linkInfo;
      function dragstarted(event, d) {
        linkInfo = Object.create(null);
        var parentData = getParentData(d.pid);
        var x = event.x + parentData.x + 2;
        var y = event.y + parentData.y + 2;
        linkInfo.id = "path-".concat(d.pid, "-") + int(0, 9999)();
        linkInfo.source = [x, y];
        selection.append("path").data([linkInfo]).attr("stroke", "#85c9d1").attr("fill", "none").attr("class", linkInfo.id).lower();
      }
      function dragged(event, d) {
        var parentData = getParentData(d.pid);
        var x = event.x + parentData.x + 2;
        var y = event.y + parentData.y + 2;
        linkInfo.target = [x, y];
        selection.select("." + linkInfo.id).attr("d", linkHorizontal());
      }
      function dragended(event, d) {
        var parentData = getParentData(d.pid);
        var x = event.x + parentData.x;
        var y = event.y + parentData.y;
        var hasLinked = selectAll(".container .inputs").filter(function (inputD) {
          var parentD = getParentData(inputD.pid);
          var xp = parentD.x + inputD.x + 2 - x;
          var yp = parentD.y + inputD.y + 2 - y;
        ...
        if (hasLinked.size() > 0) {
          ...
          var link = selection.select("." + linkInfo.id).attr("d", linkHorizontal()).lower().each(function (d) {
            d.data = {
              type: "link",
          ...
          dispatch$1.call("link", null, {
            sourceNamespace: parentData.type !== 98 ? parentData.callFunction : null,
            source: d,
      ...
      function getParentData(pid) {
        return ins.data().find(function (pd) {
          return pd.id === pid;
        });
      }
      return drag().on("start", dragstarted).on("drag", dragged).on("end", dragended);
  ...
  function TaskList (selection, tasksData, savedTasks) {
    ...
    var TaskIns = TaskNode().on("link", handleLinkEvent).on("delete", handleTaskDeleteEvent).on("update", handleUpdate);
  ...
  function ActionButtons () {
    ...
    col.className = "col";
    row.appendChild(col);
    select(col).append("button").attr("class", "pure-button").attr("type", "button").text("生成代码").on("click", function () {
      var str = generator.generate();
      console.log(str);
    });
    select(col).append("button").attr("class", "btn btn btn-primary").style("margin", "10px").attr("type", "button").text("保存状态").on("click", function () {
